...
trait CsvSupport extends CsvMarshalling {
  ...
  def csvRendering[T]()(implicit render: Marshaller[T, ByteString], mat: Materializer): Flow[T, ByteString, Unit] =
    Flow[T]
      .mapAsync(1)(in => render(in)(mat.executionContext))
      .mapConcat(identity)
      .collect {
        case Marshalling.WithFixedCharset(MediaTypes.`text/csv`, _, marshal) => marshal()
...
//    Marshaller.withFixedCharset(MediaTypes.`text/csv`, HttpCharsets.`UTF-8`) { productToCsv(_) }
  ...
  implicit def lAsCsvString(implicit mat: Materializer): Marshaller[Iterable[BS], BS] =
    Marshaller.withFixedCharset(MediaTypes.`text/csv`, HttpCharsets.`UTF-8`) { l => productToCsv(l.toList) } // TODO cuz' lazy today
